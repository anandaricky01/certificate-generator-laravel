...
class FillPDFController extends Controller
{
    public function process(Request $request)
    {
        ...
        $outputFileNomor = public_path() . "/sertif/sertifikat staff tetap_" . $validated['nama'] . "_" . $validated['nim'] . "_temp.pdf";
        $outputFileJabatan = public_path() . "\sertif\Sertifikat Staff Tetap_"  . $validated['nama'] . "_" . $validated['nim'] . ".pdf";

        if ($sertifikat->where('nim', '=', $validated['nim'])->count() == 1) {

            // check apakah nama dan nim sama dengan file yang telah disimpan?
            if (!file_exists($outputFileJabatan)) {
                $nama = $sertifikat->where('nim', '=', $validated['nim'])->get()[0]->nama;
                $peringatan = 'NIM dan Nama tidak sesuai! <br> NIM berikut bernama ' . $nama;
            ...
            $this->fillPDFJabatan(public_path() . "/sertif/sertifikat staff tetap_" . $validated['nama'] . "_" . $validated['nim'] . "_temp.pdf", $outputFileJabatan, $validated['jabatan']);
